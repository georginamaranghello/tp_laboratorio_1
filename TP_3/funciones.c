...
void agregarPelicula(EMovie movie[], int tam)
{
    ...
    printf("\nTITULO:%s\nDURACION:%s\nGENERO:%d\nDESCRIPCION:%s\nPUNTAJE:%d\nLINK DE IMAGEN:%s\n",movie[pos].titulo, movie[pos].genero, movie[pos].duracion, movie[pos].descripcion,movie[pos].puntaje, movie[pos].linkImagen);
...
int buscarPorTitulo(EMovie movie[],int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(strcmpi(movie[i].titulo,titulo)==0)
        {
            if(movie[i].estado==1)
            {
                aux=i;
